...
export default function Tile({label, content, change}: TileProps ) {
    ...
    return <Statistic style={{background: "lightblue", padding: "1em"}}>
    <Statistic.Label>{label}</Statistic.Label>
    <Statistic.Value>{value}</Statistic.Value>
    {change == null?
        <Statistic.Label>Geen gegevens</Statistic.Label>:
        <Statistic.Label style={{color: change>0?"green":"red"}}><Icon name={`arrow ${change>0?'up':'down'}`}/>{Math.round(change*100)}%</Statistic.Label>}
  </Statistic>
}